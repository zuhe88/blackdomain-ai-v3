...
async function main() {
  ...
  for (const expected of ["app.js?v=20260905.01", "styles.css?v=20260905.01", "admin.css?v=20260905.01"]) {
    if (!webPortalSource.includes(expected)) throw new Error(`Website cache-busted asset is missing: ${expected}`);
  }
  for (const expected of ["etag: false", '"cache-control", "no-store, no-cache, must-revalidate"', "web.waitReply(replyToken, 20_000)", 'portalBuild: "20260905.01"', 'isAdminLineUserId(userId)', '"/api/web/admin/monitor"']) {
