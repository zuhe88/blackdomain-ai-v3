...
function dashboard(me) {
  currentMember = me;
  activeResult = null;
  root.innerHTML = `<section class="section-head"><div><p class="eyebrow">ATG X・SETH EXCLUSIVE COMMAND</p><h1>戰神雙核心分析</h1><p>雙遊戲獨立資料鏈路，空房校驗完成後才會建立分析結果。</p></div><div class="overview-meta"><div class="overview-brand"><span>ATG / SETH INTELLIGENCE</span><b>ATG 賽特決策核心</b><small>即時房況・樣本評級・資金參數</small></div><div class="license-state">授權至 ${new Date(me.expiresAt).toLocaleDateString("zh-TW")}</div></div></section><section class="game-grid" id="games">${games.map(gameCard).join("")}</section><div id="room-status">${roomStatusBar()}</div><section class="workspace"><div class="panel command-panel"><div class="panel-kicker">ANALYSIS CONTROL</div><h2>戰術參數</h2><div class="control"><label>目前分析核心</label><input id="gameName" value="${escapeHtml(selectedGame)}" readonly></div><div class="control"><label>本次操作本金（選填）</label><input id="bankroll" inputmode="numeric" value="${escapeHtml(bankrollValue)}" placeholder="例如 10000"></div><button class="primary" id="analyze">啟動 AI 戰術掃描</button><button class="ghost" id="logout">登出本裝置</button></div><div class="result" id="result"><div class="empty-result standby"><div class="scanner-field"><div class="scanner-orbit"><i></i><span>AI</span></div></div><b class="standby-title">等待啟動戰術掃描</b></div></div></section>`;
}
...
function resultCard(result) {
  ...
  const confidenceMeter = result.confidence === "高" ? 92 : result.confidence === "中" ? 68 : 42;
  const confidenceStatus = result.confidence === "高" ? "優先評估" : result.confidence === "中" ? "持續觀察" : "暫緩評估";
  const updated = result.updatedAt ? new Date(result.updatedAt).toLocaleTimeString("zh-TW", { hour12: false }) : "—";
  return `<section class="result-summary"><div class="result-topline"><span><i></i>ANALYSIS READY</span><time>資料核對 ${escapeHtml(updated)}</time></div><div class="room-overview"><div class="room"><small>${escapeHtml(result.gameName)}・推薦房間</small><strong>${escapeHtml(result.roomNumber)}</strong><em>SELECTED ROOM</em></div><div class="confidence ${confidenceClass}" style="--meter:${confidenceMeter * 3.6}deg"><div class="confidence-ring"><b>${escapeHtml(result.confidence)}</b></div><div class="confidence-copy"><small>ROOM RANK</small><strong>推薦程度</strong><em>${escapeHtml(confidenceStatus)}</em></div></div></div><div class="data-strip"><div><span>今日得分率</span><b>${formatNumber(result.metrics.todayRtp, "%")}</b></div><div><span>近30日得分率</span><b>${formatNumber(result.metrics.monthRtp, "%")}</b></div><div><span>今日總下注額</span><b>${formatNumber(result.metrics.todayBet)}</b></div></div><div class="room-actions"><span>符合條件：共有 ${formatNumber(result.availableRooms)} 房</span><button class="ghost" id="recheck-room">重新核對目前房間</button></div></section>${playbookPanel(result)}<p class="note">${escapeHtml(result.note)}</p>`;
}
