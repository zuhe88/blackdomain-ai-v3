...
function unavailableLoginPage() {
  return `<!doctype html><html lang="zh-Hant"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="referrer" content="no-referrer"><title>登入服務暫時忙碌</title><style>body{margin:0;min-height:100vh;display:grid;place-items:center;background:#080807;color:#fff;font-family:system-ui,-apple-system,"Noto Sans TC",sans-serif}.card{width:min(88vw,390px);padding:34px 25px;text-align:center;border:1px solid #806415;border-radius:24px;background:#12110e}.brand{color:#f3cc39;font-size:12px;letter-spacing:.12em}h1{font-size:24px;margin:12px 0}p{color:#bbb5a5;line-height:1.7}a{display:block;margin-top:22px;padding:13px;border-radius:14px;background:#dfb426;color:#171207;text-decoration:none;font-weight:800}</style></head><body><main class="card"><div class="brand">BLACKDOMAIN AI</div><h1>登入服務暫時忙碌</h1><p>系統無法即時完成安全登入。<br>請回到 LINE 再傳送一次「網站登入」。</p><a href="/portal/">返回網站首頁</a></main></body></html>`;
}
...
function registerWebPortalRoutes(app) {
  ...
  app.post("/portal/login", async (req, res, next) => {
    ...
    try {
      ...
      console.error("[WebPortal] Login redemption failed:", error.message);
      return res.status(503).type("html").send(unavailableLoginPage());
    }
    res.setHeader("cache-control", "no-store");
    res.setHeader("x-robots-tag", "noindex, nofollow, noarchive");
